...
WITH source AS (
  SELECT * FROM  {{source(var('source_schema', 'DEMO_SQUARE_ALT13'),'ORDER_LINE_ITEM_MODIFIER')}}
),
source_order AS (
...
source_catalog_modifier AS (
  SELECT * FROM  {{source(var('source_schema', 'DEMO_SQUARE_ALT13'),'CATALOG_MODIFIER')}}
),
