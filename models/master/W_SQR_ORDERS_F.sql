...
WITH source_item_allocated as (
  select 
        ...
        ,OLI.K_POS_ORDER_BK
        FROM 
        {{ref('V_SQR_ORDER_LINE_ITEM_STG')}} OLI
        LEFT JOIN {{ref('V_SQR_ORDER_LINE_ITEM_MODIFIER_STG')}} OLIM ON OLIM.K_POS_ORDER_LINE_ITEM_BK = OLI.K_POS_ORDER_LINE_BK AND OLIM.K_POS_ORDER_BK = OLI.K_POS_ORDER_BK
...
source_order_allocated as (
  select MAX(O.M_NET_TIP_MONEY_AMT)/COUNT(1) AS M_NET_TIP_MONEY_AMT
        ...
        {{ref('V_SQR_ORDER_HEADER_STG')}} O
        LEFT JOIN {{ref('V_SQR_ORDER_LINE_ITEM_STG')}} OLI ON OLI.K_POS_ORDER_DLHK = O.K_POS_ORDER_DLHK AND OLI.K_POS_ORDER_BK = O.K_POS_ORDER_BK
        LEFT JOIN {{ref('V_SQR_ORDER_LINE_ITEM_MODIFIER_STG')}} OLIM ON OLIM.K_POS_ORDER_LINE_ITEM_BK = OLI.K_POS_ORDER_LINE_BK AND OLIM.K_POS_ORDER_BK = OLI.K_POS_ORDER_BK
...
SELECT
  ...
  ,O.MD_INTGR_ID
FROM
  {{ref('V_SQR_ORDER_HEADER_STG')}} AS O 
  INNER JOIN {{ref('V_SQR_ORDER_LINE_ITEM_STG')}} OLI ON OLI.K_POS_ORDER_DLHK = O.K_POS_ORDER_DLHK
  LEFT JOIN {{ref('V_SQR_ORDER_LINE_ITEM_MODIFIER_STG')}} OLIM ON OLIM.K_POS_ORDER_LINE_ITEM_BK = OLI.K_POS_ORDER_LINE_BK AND OLIM.K_POS_ORDER_BK = OLI.K_POS_ORDER_BK
